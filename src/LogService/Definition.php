...
class Definition
{
    ...
    //         'LOG %s:%s', $this->app->config['access_key_id'], $this->calculateSignature($method, $contentType, $date, $canonicalizedHeaders, $resource.'?'.$str, 'sha1')
    //         // 'LOG %s:%s', $this->app->config['access_key_id'], $this->calculateSignature($method, $contentType, $date, $canonicalizedHeaders, '/logstores/idontknow?'.$str, 'sha1')
    //     );
